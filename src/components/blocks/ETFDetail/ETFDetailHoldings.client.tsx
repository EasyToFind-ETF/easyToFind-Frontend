...
const ETFDetailHoldings: React.FC<ETFDetailHoldingsProps> = ({ etf_code }) => {
  ...
  const getPieChartData = () => {
    ...
    const pieData = topHoldings.map(
      (holding: ETFHoldingsWithStock, index: number) => {
    ...
    if (otherHoldings.length > 0) {
      const otherWeight = otherHoldings.reduce(
        (sum: number, holding: ETFHoldingsWithStock) =>
          sum + holding.weight_pct,
        0
      );
      pieData.push({
        id: "기타",
  ...
  return (
    <>
      ...
      <div id="holdings" className="mx-auto w-full flex-1 min-w-0 pl-10 py-14">
        <div
          className="bg-white rounded-3xl w-full px-16 py-16 shadow overflow-visible"
        ...
        >
          <div>
            ...
            {/* 파이 차트와 리스트 */}
            {!isLoading && !isError && pieChartData.length > 0 && (
              <div className="flex justify-start mb-8 gap-8">
                <div style={{ width: "500px", height: "400px" }}>
                  <ResponsivePie
                    ...
                    arcLinkLabel={(d: any) => {
                      ...
                      const stockCode =
                        (d.data as any).stockCode || d.data.id.split("_")[0];
                      const holding = holdingsData.find(
                        (h: ETFHoldingsWithStock) => h.stock_code === stockCode
                      );
                          ...
                          >
                            ...
                            >
                              ...
                              <span style={{ whiteSpace: "nowrap" }}>
                                <strong>기타</strong>: {datum.value.toFixed(2)}%
                              </span>
                            </div>
                          </div>
                        );
                ...
                <div className="flex-1">
                  <div className="text-base font-medium text-gray-700 whitespace-nowrap text-left ml-10 mb-8">
                    구성종목 Top 10
                  </div>
                  <div className="space-y-3 ml-6">
                    {pieChartData
                      .filter((item: any) => item.id !== "기타")
                      .map((item: any, index: number) => {
                        // stockCode를 사용해서 종목명 찾기
                        const stockCode =
                          (item as any).stockCode || item.id.split("_")[0];
                        const holding = holdingsData.find(
                          (h: ETFHoldingsWithStock) =>
                            h.stock_code === stockCode
                        );
                        return (
                          <div
                            key={`${item.id}-${index}`}
            ...
            <div className="mt-20">
              ...
              {holdingsData.length > 0 ? (
                <>
                  ...
                  {/* 더보기 버튼 */}
                  {holdingsData.length > 10 && (
                    <div className="flex justify-center mt-10">
                      <button
                        onClick={() => setShowAllHoldings(!showAllHoldings)}
                        className="px-6 py-3 bg-gray-200 text-black rounded-2xl hover:bg-gray-300 transition-colors duration-200 font-medium"
                      >
                        {showAllHoldings
                          ? "접기"
                          : `더보기 (${holdingsData.length - 10}개 더)`}
                      </button>
                    </div>
                  )}
                </>
