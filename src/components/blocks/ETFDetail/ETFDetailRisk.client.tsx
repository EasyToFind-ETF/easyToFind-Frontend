...
const ETFDetailRisk: React.FC<ETFDetailRiskProps> = ({ etf_code }) => {
  ...
  const getRiskColor = (riskValue: number): string => {
    switch (riskValue) {
      case 1: return "#FF0000"; // 빨간색 (가장 위험)
      case 2: return "#FF5A3D"; // 주황색 (위험)
      case 3: return "#FFA500"; // 오렌지색 (보통)
      case 4: return "#32CD32"; // 초록색 (안전)
      case 5: return "#008000"; // 진한 초록색 (가장 안전)
  ...
  // 실제 데이터 사용 (기본값 제거)
  const score = riskData ? parseNumber(riskData.score) : 0;
  ...
  const volatilityPieData = createPieData(volatility * 100, 50, volatilityColor); // Volatility 최대값 50%로 가정
                      ...
                      ({ centerX, centerY }) => (
                        <>
                          <text
                            ...
                            style={{ fontSize: 40, fill: riskColor, fontWeight: 600 }}
                          >
                            {riskValue}
                          </text>
                          <text
                            ...
                            textAnchor="middle"
                            dominantBaseline="central"
                            style={{ fontSize: 16, fill: riskColor, fontWeight: 600 }}
                          >
                            {riskLabel}
                          </text>
                        </>
                      ...
                      ({ centerX, centerY }) => (
                        <>
                          <text
                            ...
                            textAnchor="middle"
                            dominantBaseline="central"
                            style={{ fontSize: 40, fill: mddColor, fontWeight: 600 }}
                          >
                            {mdd.toFixed(1)}
                          </text>
                          <text
                            ...
                            textAnchor="middle"
                            dominantBaseline="central"
                            style={{ fontSize: 16, fill: mddColor, fontWeight: 600 }}
                          >
                            MDD(%)
                          </text>
                        </>
                      ...
                      ({ centerX, centerY }) => (
                        <>
                          <text
                            ...
                            textAnchor="middle"
                            dominantBaseline="central"
                            style={{ fontSize: 40, fill: volatilityColor, fontWeight: 600 }}
                          >
                          ...
                          <text
                            ...
                            textAnchor="middle"
                            dominantBaseline="central"
                            style={{ fontSize: 16, fill: volatilityColor, fontWeight: 600 }}
                          >
                            변동성(%)
                          </text>
                        </>
