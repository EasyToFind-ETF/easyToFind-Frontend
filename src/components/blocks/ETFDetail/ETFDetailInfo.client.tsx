...
const ETFDetailInfo: React.FC<ETFDetailInfoProps> = ({ etf_code }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (etfResult.status === 200 && etfResult.data) {
          ...
          } else if (etfResult.data.daily_prices && etfResult.data.daily_prices.length > 0) {
  ...
  const { aum, mkt_cap, list_shrs, acc_trd_val, idx_ind_nm } = dailyPriceData || {};
        ...
        <div className="border border-gray-200 rounded-3xl overflow-hidden" style={{ borderRadius: '2rem' }}>
          ...
          <div className="w-full px-10 py-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 text-base bg-white">
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">순자산 총액</span>
              <span className="font-semibold text-right ml-4">
                {aum ? `${Math.round(Number(aum) / 100000000).toLocaleString()}억원` : '-'}
              </span>
            </div>
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">상장일</span>
              <span className="font-semibold text-right ml-4">
                {inception_date ? new Date(inception_date).toLocaleDateString('ko-KR') : '-'}
              </span>
            </div>
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">누적거래대금</span>
              <span className="font-semibold text-right ml-4">
                {acc_trd_val ? `${Math.round(Number(acc_trd_val) / 100000000).toLocaleString()}억원` : '-'}
              </span>
            </div>
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">운용 보수</span>
              <span className="font-semibold text-right ml-4">
                {expense_ratio || '-'}%
              </span>
            </div>
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">시가총액</span>
              <span className="font-semibold text-right ml-4">
                {mkt_cap ? `${Math.round(Number(mkt_cap) / 100000000).toLocaleString()}억원` : '-'}
              </span>
            </div>
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">상장주식수</span>
              <span className="font-semibold text-right ml-4">
                {list_shrs ? `${Math.round(Number(list_shrs) / 10000).toLocaleString()}만주` : '-'}
              </span>
            </div>
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">자산운용사</span>
              <span className="font-semibold text-right ml-4">
                {provider || '-'}
              </span>
            </div>
            <div className="flex justify-between items-center bg-[#F2F8FC] rounded-xl px-6 py-3 min-h-[60px]">
              <span className="text-gray-500 text-sm whitespace-nowrap">지수산업명</span>
              <span className="font-semibold text-right ml-4">
                {idx_ind_nm || '-'}
              </span>
            </div>
