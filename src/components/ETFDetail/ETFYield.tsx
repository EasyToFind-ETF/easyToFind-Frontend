...
const ETFYield: React.FC = () => {
  ...
  return (
    <div id="yield" className="max-w-[1600px] mx-auto w-full flex-1 min-w-0 pl-10 pt-14">
      <div className="bg-white rounded-3xl w-full px-20 py-20 shadow" style={{ borderRadius: '4rem' }}>
        {/* 타이틀, 구분선, 기준일 */}
        <div>
          <div className="text-4xl font-semibold mb-10">수익률</div>
          <hr className="border-b-2 border-gray-200 mb-10" />
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-4 w-full">
              <span className="font-semibold ml-3 mr-7 whitespace-nowrap">적용기간</span>
              <div className="flex bg-[#F6F7F9] rounded-2xl p-1 w-full">
                {periods.map((period) => (
                  <button
                    ...
                    style={activePeriod === period ? { borderColor: '#0046ff', color: '#0046ff' } : {}}
                  >
                    {period}
                  </button>
                ))}
        ...
        {/* 범례 */}
        <div className="flex gap-20 bg-[#F6F7F9] rounded-2xl px-6 py-4 text-base items-center justify-center">
          <div className="flex items-center gap-2">
            <span className="w-4 h-4 rounded bg-red-400 inline-block"></span>
            <span className="font-medium">기준가격(NAV)</span>
            <span className="text-gray-500 text-sm">-1.76%(4,634.78원)</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-4 h-4 rounded bg-cyan-400 inline-block"></span>
            <span className="font-medium">시장가격(종가)</span>
            <span className="text-gray-500 text-sm">-2.63%(4,625원)</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-4 h-4 rounded bg-green-300 inline-block"></span>
            <span className="font-medium">기초지수</span>
            <span className="text-gray-500 text-sm">-1.62%(1,312.62pt)</span>
          </div>
        </div>
        ...
        <div className="mt-16">
          <div className="text-3xl font-semibold mb-10">기간별 수익률</div>
          <hr className="border-b-2 border-gray-200 mb-10" />
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-center border-separate border-spacing-0">
              <thead>
                <tr className="bg-[#F6F7F9] text-gray-700 text-base">
